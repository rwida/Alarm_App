...
class OrderState extends State<OrderFilter>{
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(actions: [
       // IconButton(icon: Icon(Icons.tune,color: Color(0xFFff8500),), onPressed: (){
        //  _selectDate(context);
        ...
        // );
      //})
      ],centerTitle: true,elevation: 0,title:  Text("All Orders",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold,fontSize: MediaQuery.of(context).size.height*0.025),),backgroundColor: Color(0xFF284456),),
                             ...
                             // SizedBox(height: 7.0),
                              Row(children: [
                                Row(children: [
                                  ...
                                  Text(s.length>1?' Products':' Product',style: TextStyle(fontWeight: FontWeight.bold,fontSize: MediaQuery.of(context).size.height*0.015,color: Color(0xFFff8500)),),
                                ],),
                                //SizedBox(width: 10,),
  ...
  Future<void> _displayTextInputDialog(BuildContext context) async {
   ...
   void getcomments() async {
     String username = '7712bd6b62936aac7664b79616da13e2';
     String password = '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';
     String basicAuth =
         'Basic ' + base64Encode(utf8.encode('$username:$password'));
     http.Response response = await http.get('https://superkshop.myshopify.com/admin/api/2020-10/orders.json?limit=50',headers: {"Content-Type": "application/json",
  ...
  void getCategorys() async {
    ...
    for(int i=0;i<map['orders'].length;i++){
      if(map['orders'][i]['email'].toString() == 'm.nabil1309@gmail.com' && map['orders'][i]['created_at'].toString().substring(0,10) == widget.InvDate){
        setState(() {
