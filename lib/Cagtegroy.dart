...
class Cagtegroy extends StatefulWidget{
  final int CatId,MainCategroy,SellerId;
  Cagtegroy({this.CatId,this.MainCategroy,this.SellerId});
  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return CagtegroyState();
  }

}
class CagtegroyState extends State<Cagtegroy>{
  int active = 0;
  int load = 0;
  String SellerName = '';
  String SellerName_ar = '';
  List cc = [];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(actions: [IconButton(icon: Icon(Icons.search,color: Color(0xFFff8500),), onPressed: (){
        showSearch(
            context: context,
            delegate: DataSearch(ListNew,widget.CatId,widget.MainCategroy,widget.SellerId));
      })],iconTheme: IconThemeData(
        color: Color(0xFFff8500), //change your color here
      ),centerTitle: true,elevation: 0,title:  Text(AppLocalizations.of(context).Products,style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold,fontSize: MediaQuery.of(context).size.height*0.025),),backgroundColor: Colors.white,),
                  ...
                  child: Column(
                    children: <Widget>[
                      Container(
                        ...
                        //  color: active == id ?Color(0xFF284456):Color(0xFFf8f8f8),
                       ...
                       child: SubCate[id]['photo'].toString().isEmpty?Icon(Icons.category_outlined,color:active == id ?Color(0xFFff8500):Colors.grey ,):Image.network("${SubCate[id]['photo']}"),
                        //child: Icon(Icons.category_outlined,color:active == id ?Color(0xFFff8500):Colors.grey ,),
                      ),
                      Text(Langs=="ar"?"${SubCate[id]['sub_category_name_ar']}":"${SubCate[id]['sub_category_name']}",
                        //"${SubCate[id]['sub_category_name']}",
              ...
              Container(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.start,
                  children: <Widget>[
                    ...
                    Text(Langs=="ar"?"${Products[i]['name_ar']}":"${Products[i]['product_name']}",
                    ...
                    Products[i]['compare_price']==0?Container():Row(children: [
  ...
  void getSubCategorys() async {
    ...
    http.Response response = await http.get('http://wootc.net/api/subCategories?access_token=syEt93K5Gsg2Ozdgv8oK22NQLYNzMJtiLHD06JMrpkqF05Kc5VCl9aeWgpcdAB6r',headers: {"Accept":"application/json"},);
    Map map = json.decode(response.body);
    // print(widget.CatId.toString());
    print(map);
    if(map['Status'] == true){
      SubCate.clear();
      for(int i =0;i<map['Sub Categories'].length;i++){
        if(map['Sub Categories'][i]['category_id']==widget.CatId){
          setState(() {
            SubCate.add(map['Sub Categories'][i]);
          });
        }
      }
  ...
  void getProducts(subId) async {
    ...
    if(map['Status'] == true){
      ...
      for(int i =0;i<map['Products'].length;i++){
        if(map['Products'][i]['category_id']==widget.CatId && map['Products'][i]['sub_category_id']==subId && map['Products'][i]['main_category_id']==widget.MainCategroy && map['Products'][i]['seller_id']==widget.SellerId){
          setState(() {
  ...
  void getSellerName () async {
    ...
    Map map = json.decode(response.body);
    print(map);
    for(int y = 0;y<map['Sellers'].length;y++){
      if(map['Sellers'][y]['id'] == widget.SellerId){
        setState(() {
          SellerName = map['Sellers'][y]['seller_name'];
          SellerName_ar = map['Sellers'][y]['seller_ar'];
        });
      }
    }
  }
...
class DataSearch extends SearchDelegate<String> {
  DataSearch(this.items,this.CatId,this.MainCategroy,this.SellerId);
  final int CatId,MainCategroy,SellerId;
            ...
            return GestureDetector(
              child:  ListTile(
                ...
                subtitle: Row(children: [
                  ...
                  //   // size: 30,
                  // ),
                  Text("${NumberFormat.currency(name: '').format(double.parse(suggestionList[index].price))}  "+AppLocalizations.of(context).LE,style: TextStyle(
                      fontWeight: FontWeight.w500,
                      fontSize: MediaQuery.of(context).size.height*0.02,color: Colors.black
                  ),),

                  SizedBox(width: 10,),
                  suggestionList[index].priceC=="0"?Container():Text("${NumberFormat.currency(name: '').format(double.parse(suggestionList[index].priceC))}  "+AppLocalizations.of(context).LE,style: TextStyle(
                      fontSize: MediaQuery.of(context).size.height*0.015,
                      fontWeight: FontWeight.w500,
                      //color: warning,
                      color: Color(0xFFff8500),
                ...
                trailing: (suggestionList[index].priceC=="0")?Column(children: [
